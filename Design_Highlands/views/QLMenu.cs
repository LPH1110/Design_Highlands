...
namespace Design_Highlands
{
    internal partial class QLMenu: Form
    {
        ...
        private void loadDrinks()
        {
            ...
            IMongoCollection<BsonDocument> collection1 = db.GetCollection<BsonDocument>("foods");
            List<BsonDocument> results = collection.Find(new BsonDocument()).ToList();
            List<BsonDocument> results1 = collection1.Find(new BsonDocument()).ToList();
            ...
            foreach (BsonDocument result in results1)
            {
                Food food = BsonSerializer.Deserialize<Food>(result);
                table.Rows.Add(food.Id, food.Type, food.NameE, food.NameV, food.Price);
            }
            menuGridView.DataSource = table;

            /*
            string selectedType = cbb_menuOptions.SelectedItem.ToString();
            ...
            {
                DataTable filteredTable = table.AsEnumerable().Where(row => row.Field<string>("Class") == selectedType).CopyToDataTable();
                menuGridView.DataSource = filteredTable;
            }
            */
        }
        private void cbbType_SelectIndexChanged(object sender, EventArgs e, DataTable table)
        {
